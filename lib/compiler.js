  ...
  if (!Array.isArray(entry) && typeof entry === 'object') {
    ...
    for (const entryName of keys) {
      if (options.allEntries) {
        newEntry[entryName] = {
          import: clientEntry.concat(entry[entryName].import),
        };
        ...
        newEntry[entryName] = {
          import: entry[entryName].import,
        };
      }
    }
  } else {
    newEntry = {
      import: clientEntry.concat(entry.import),
    };
  }
  return newEntry;
}
      ...
      comp.hooks.beforeCompile.tap('WebpackHotClient', () => {
        if (!comp.hotClient.optionsDefined) {
          ...
          const definePlugin = new DefinePlugin({
            __hotClientOptions__: stringify({
              logLevel,
              hmr,
              reload,
              webSocket,
